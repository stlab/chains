...
// configuration step. It creates a namespace called `chains`. You can modify
    ...
    const auto make_buttons = [&] {
        ...
        for (std::size_t cur_x = 0; cur_x < game_board.width; ++cur_x) {
            for (std::size_t cur_y = 0; cur_y < game_board.height; ++cur_y) {
                buttons.push_back(
                    ftxui::Button(&game_board.get_string(cur_x, cur_y), [=, &game_board] {
...
int main(int argc, const char** argv) {
    try {
        CLI::App app{fmt::format("{} version {}", chains::cmake::project_name,
                                 chains::cmake::project_version)};
